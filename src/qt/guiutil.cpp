...
{
    // c0fferCoin: check prefix
    if(uri.scheme() != QString("c0fferCoin"))
        return false;
...
{
    // Convert c0fferCoin:// to c0fferCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("c0fferCoin://"))
    {
        uri.replace(0, 12, "c0fferCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "c0fferCoin.lnk";
}
...
{
    return GetAutostartDir() / "c0fferCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("c0fferCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  c0fferCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("c0fferCoin-Qt"));
